...
async def init(config: dict):
    curve_lending_bot_vyper = config['VYPER']
    curve_lending_bot_abi = json.loads(config['ABI'])
    payload = ""
    ...
    result = await paloma.job_scheduler.create_job(
        wallet, job_id, curve_lending_bot_vyper, curve_lending_bot_abi,
        payload, chain_type, chain_reference_id, creator, signers)
    print(result)
    time.sleep(6)
